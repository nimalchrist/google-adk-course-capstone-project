...
## Overview
eComBot is a full-featured e-commerce support agent built with Google ADK, demonstrating all concepts from Day 01 through Day 08 of the Google ADK course.

## Features by Day

| Day | Feature | Implementation |
|-----|---------|----------------|
| 01-02 | Base agent + prompt refinement | `src/agents/support_agent.py`, instruction files |
| 03 | Tool calling + in-memory state | `src/tools/order_tools.py`, session state |
| 04 | Redis + PostgreSQL persistence | `src/services/`, `docker-compose.yml` |
| 05-06 | RAG with ChromaDB | `src/rag/`, `data/` knowledge base |
| 07 | LiteLLM routing + fallback | `src/routing/router.py` |
| 08 | FastMCP external tool servers | `mcp_servers/` |
...
### 3. Start infrastructure (optional, for Day 04 features)
```bash
docker compose up -d
```
